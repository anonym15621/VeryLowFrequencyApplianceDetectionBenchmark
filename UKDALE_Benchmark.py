...
def launch_deep_training(model, X_train, y_train, X_valid, y_valid, X_test, y_test, path_res
                         max_epochs=100, batch_size, lr=1e-3, weight_decay=1e-3):
    X_train, y_train = RandomUnderSampler_(X_train, y_train)
    ...
    model_trainer = classif_trainer(model(),
...
def UKDALE_case(chosen_clf, classifiers, list_case, list_param, path_res):
    ...
    for case in list_case:
        dir_res_case = path_res+case+os.sep
        create_dir(dir_res_case)
        for param in list_param:
            dir_res_param = dir_res_case+param['sampling_rate']+os.sep
            create_dir(dir_res_param)
            databuilder = UKDALEData(appliance_names=[case],
            ...
            for seed in range(5):
                ...
                X_train, y_train = TransformNILMDatasettoClassif(train, appl_threshold=0)
                X_train, X_valid, y_train, y_valid = train_test_split(X_train, y_train, test_size=0.2, random_state=seed)
                X_test, y_test = TransformNILMDatasettoClassif(test, appl_threshold=0)
                
                path_to_save = dir_res_param+chosen_clf+'_'+str(seed)

                if chosen_clf=="Inception":
                    path_inception = path_to_save+os.sep
                    create_dir(path_inception)
                ...
                elif chosen_clf=="ResNet" or chosen_clf=="ConvNet" or chosen_clf=="ConvResNetAttention":
...
if __name__ == "__main__":
    ...
    list_case = ['washing_machine', 'microwave', 'dishwasher', 'kettle']
